...
router.post('/register', validateRegistration, validateRequest, async (req: Request, res: Response) => {
  try {
    const { 
      ...
      phone,
      address,
      property_id, // Property ID for staff registration
    ...
    let finalPropertyId: number | null = null;
    ...
    if (requestedRole === 'staff') {
      ...
      if (property_id) {
        ...
        console.log(`[Register] Staff ${email} registered with property_id: ${finalPropertyId}`);
      } else {
        console.log(`[Register] Staff ${email} registered without property. Will need manual assignment.`);
      }
    }
