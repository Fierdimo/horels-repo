...
import { Bed, Edit, Trash2, Search, Filter, X, RefreshCw, Calendar } from 'lucide-react';
...
export default function StaffInventory() {
  ...
  const [floorFilter, setFloorFilter] = useState<string>('all'); // V1 concept - not used in V2
  ...
  const filteredRooms = rooms.filter((room: any) => {
    const matchesSearch =
      ...
      (room.propertyCity || '').toLowerCase().includes(searchQuery.toLowerCase());
    ...
    const matchesProperty =
      propertyFilter === 'all' ||
      (room.propertyName || '') === propertyFilter;

    const matchesDate = 
      !dateFilter ||
      (room.weekInfo?.startDate && new Date(room.weekInfo.startDate) >= new Date(dateFilter));

    const matchesCapacity =
      capacityFilter === 'all' ||
      room.capacity === parseInt(capacityFilter);

    return matchesSearch && matchesStatus && matchesType && matchesProperty && matchesDate && matchesCapacity;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('nav.inventory') || 'Available Inventory'}</h1>
          <p className="text-gray-600 mt-1">
            {t('staff.inventory.subtitle') || 'Manage available weeks and marketplace inventory'}
          </p>
        </div>
      ...
      {showFilters && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {(statusFilter !== 'all' || typeFilter !== 'all' || propertyFilter !== 'all' || capacityFilter !== 'all' || searchQuery || dateFilter) && (
              <button
                onClick={() => {
                  setStatusFilter('all');
                  setTypeFilter('all');
              ...
              >
                ...
                {t('admin.rooms.clearFilters') || 'Clear Filters'}
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder={t('common.searchPlaceholder') || 'Search by name, location...'}
                  value={searchQuery}
          ...
          {(searchQuery || propertyFilter !== 'all' || dateFilter || typeFilter !== 'all' || capacityFilter !== 'all') && (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {capacityFilter !== 'all' && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm">
                    ...
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setCapacityFilter('all')} />
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-4" style={{ display: 'none' }}>
        <div className="flex items-center justify-between mb-4">
